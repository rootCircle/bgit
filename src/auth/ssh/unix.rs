...
/// Start SSH agent detached from current process
pub fn start_agent_detached(socket: Option<&Path>) -> Result<(), Error> {
...
/// Start ssh-agent and parse environment variables from its output
...
/// Add all available SSH keys to the agent
...
/// Try SSH key files directly without agent
...
/// Get the count of identities in SSH agent
...
/// Interactively add a key to SSH agent
