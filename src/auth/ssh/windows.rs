...
/// Ensure an ssh-agent is available on Windows.
/// On Windows, rely on existing agent (Pageant or OpenSSH agent).
...
/// Start SSH agent detached on Windows
pub fn start_agent_detached() -> Result<(), Error> {
...
/// Add all available SSH keys to the agent on Windows
pub fn add_all_ssh_keys(cfg: &BGitGlobalConfig) -> Result<Option<PathBuf>, Error> {
    debug!("Adding all SSH keys from .ssh folder to ssh-agent (Windows)");
...
/// Try SSH key files directly without agent (Windows)
pub fn try_ssh_key_files_directly(username: &str) -> Result<git2::Cred, Error> {
    debug!("Trying SSH key files directly for user: {username} (Windows)");
...
/// Get the count of identities in SSH agent (Windows)
...
/// Interactively add a key to SSH agent (Windows)
pub fn add_key_interactive(key_path: &Path, key_name: &str) -> Result<bool, Error> {
    debug!("Trying interactive ssh-add for key: {key_name} (Windows)");
